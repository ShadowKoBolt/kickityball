class Team < Struct.new(:slug, :name)
  ...
  def fixtures
    Rails.cache.fetch("teams/#{slug}/fixtures", expires_in: 12.hours) do
      ...
      html = Nokogiri::HTML(response.body)
      html.css("div.fixtures-table table.table-stats tbody tr").collect do |fixture_row|
        ...
        Fixture.new(date, time, home_team, away_team)
      end.compact
    end
  end
  ...
  class Fixture < Struct.new(:date, :time, :home_team, :away_team)
    def name
